...
import { useRoqPlatformUserProfiles, usePassengerCount, useFlightCount, useBookingCount } from 'lib/roq';

import { TravelAgentListPage as ShowCaseList } from 'pages/travel-agents';

function DashboardPage() {
  const { session } = useSession();

  const passengerStat = usePassengerCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Passengers',
      result: passengerStat?.data,
      isLoading: passengerStat.isLoading,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Travel Agents
                </Text>
                <NextLink href={`travel-agents`} passHref legacyBehavior>
                  <Button
